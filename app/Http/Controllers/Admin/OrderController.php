...
public function details(Request $request, $id)
    {
        ...
        if (isset($order)) {
            ...
            $newRequestq = new Request([
                'referenceCode' => $order->referenceCode,
            ]);
           
        $getorder_auth = $this->get_order_from_authority($newRequestq);
        $responsedata = json_decode($getorder_auth->getContent(), true);
        $order_outh = $responsedata['data'];
        
        // dd($order_outh);

        if (isset($responsedata['status']) && $responsedata['status'] === true) {

            return view('admin-views.order.order-view', compact('order', 'order_outh', 'deliveryMen', 'categories', 'products', 'category', 'keyword', 'editing'));
        }
     ...
     public function get_order_from_authority(Request $request)    {
        ...
        $response = Http::post('https://demo-apitawseel.naql.sa/api/Order/get', [
            'credential' => [
                'companyName' => 'master-taer',
                'password' => 'FRgQKCtIDClc',
            ],

                'refrenceCode' => $request->referenceCode,
    ...
    public function status(Request $request)
    {
        ...
        if ($request->order_status == 'delivered') {
            ...
            $cancelorder = Order::where('id', $request['id'])->first();
            $CancelRequest = new Request([
                'referenceCode' => $cancelorder->referenceCode,
            ]);
            $cancorder = $this->cancelOrder($CancelRequest);

            $responsedata = json_decode($cancorder->getContent(), true);
        ...
        return back();
          ...
          if($request->order_status == 'confirmed') {
            $acceptorder = Order::where('id', $request['id'])->first();
            $newRequest = new Request([
                'referenceCode' => $acceptorder->referenceCode,
            ]);

        $acceptedOrder = $this->acceptOrder($newRequest);

        $responsedata = json_decode($acceptedOrder->getContent(), true);
if ((isset($responsedata['status']) && $responsedata['status'] === true) || isset($responsedata['errorCodes'])) {
        ...
        Toastr::warning(translate('messages.you_can_not_change_the_status_of_this_order_now'));
        return back();
    }
            ...
            if($request->order_status == 'delivered'){
             $executeorder = Order::where('id', $request['id'])->first();

                $executeRequest = new Request([
                    'referenceCode' => $executeorder->referenceCode,
                    'deliveryPrice' => $executeorder->delivery_charge,
                    'price' => $executeorder->referenceCode,
                    'priceWithoutDelivery' => $executeorder->referenceCode,
                    'paymentMethodId' => $executeorder->referenceCode,
                    'executionTime' => $executeorder->referenceCode,
                    'driverIncome' => $executeorder->referenceCode,
                ]);
    
    
     
            $executeOrder = $this->ExecuteOrder($executeRequest);
            $responsedata = json_decode($executeOrder->getContent(), true);
            ...
            Toastr::error(trans('' . $finalErrorMessage));
                    return back();
                }
            }
...
public function cancelOrder(Request $request)
    ...
    $request->validate([
        ...
        $cancelationReasonId = 'Nap4gA1tyeY=';
        // dd($cancelationReasonId,$request->referenceCode);
        $response = Http::post('https://demo-apitawseel.naql.sa/api/Order/cancel', [
            'credential' => [
                'companyName' => 'master-taer',
                'password' => 'FRgQKCtIDClc',
            ],
         ...
         if ($response->json()['errorCodes']){
            $errorCode = $response['errorCodes'];
            $errorCode = implode(', ', $errorCode);
            $finalErrorMessage = __('messages.error_' . $errorCode, ['default' => 'Unknown error']);
            Toastr::error(translate('' . $finalErrorMessage));
           return back();
 }
  ...
  public function ExecuteOrder(Request $request)
    {
         ...
         'driverIncome' => 'required',
         //####################
        ]);
        $priceWithoutDelivery=$request->price-$request->delivery_charge;
        $paymentMethodId = 'NV25GlPuOnQ=';
        
        $response = Http::post('https://demo-apitawseel.naql.sa/api/Order/execute', [
           'credential' => [
                'companyName' => 'master-taer',
                'password' => 'FRgQKCtIDClc',
            ],
            ...
            'orderExecutionData'=> [
                ...
                'deliveryPrice' => $request->delivery_charge,
                'priceWithoutDelivery' => $priceWithoutDelivery,
                'price' => $request->price,

                'paymentMethodId' => $paymentMethodId,
                'executionTime' => now()->setTimezone('UTC')->format('Y-m-d\TH:i:s.v\Z'),

                'driverIncome' => $request->original_delivery_charge,  
            ]
...
public function acceptOrder(Request $request)    {
        ...
        $response = Http::post('https://demo-apitawseel.naql.sa/api/Order/accept', [
            'credential' => [
                'companyName' => 'master-taer',
                'password' => 'FRgQKCtIDClc',
            ],
                ...
                'referenceCode' => $request->referenceCode,
                'acceptanceDateTime' => now()->setTimezone('UTC')->format('Y-m-d\TH:i:s.v\Z'),
        ]);
        
          if ($response->successful()) {
            $status = $response->json()['status'];
            $errorCodes = $response->json()['errorCodes'] ?? 'No error code provided'; 
            return response()->json([
                'status' => $status,
                'errorCodes' => $errorCodes
                ]);
    ...
    public function add_delivery_man($order_id, $delivery_man_id)
    {
        ...
        if ($deliveryman) {
            ...
            if ($order->delivery_man) {
                ...
                $idNumber = \App\Models\DeliveryMan::where('id', $delivery_man_id)->first();
            ...
            $acceptedOrder = $this->assignDriverOrder($orderRequest);
        ...
        if (isset($responsedata['status']) && $responsedata['status'] === true) {
            ...
            $errorCode = implode(', ', $errorCode);
            $finalErrorMessage = __('messages.error_' . $errorCode, ['default' => 'Unknown error']);
        return response()->json(['message' => $finalErrorMessage], 400);
             return back();
        }
    ...
    public function assignDriverOrder(Request $request)    {
        ...
        $response = Http::post('https://demo-apitawseel.naql.sa/api/Order/assign-driver-to-order', [
            'credential' => [
                'companyName' => 'master-taer',
                'password' => 'FRgQKCtIDClc',
            ],

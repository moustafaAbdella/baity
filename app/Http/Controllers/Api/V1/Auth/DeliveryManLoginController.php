...
class DeliveryManLoginController extends Controller
{
    ...
    if (isset($responsedata['status']) && $responsedata['status'] === true)
    {
        ...
        $dm->identity_number = $request->idNumber;
        $dm->identity_type = $identity_type;
    ...
    $responseD = response()->json($responsedata);

    // فك محتوى الاستجابة وتحويله إلى مصفوفة
    $responseDataArray = json_decode($responseD->getContent(), true);
    
    // الوصول إلى errorCodes
    $errorCode = $responseDataArray['errorCodes'];
    $errorCode = implode(', ', $errorCode);
    
    // إنشاء رسالة الخطأ
    $finalErrorMessage = __('messages.error_' . $errorCode, ['default' => 'Unknown error']);
    return response()->json(['message' => trans($finalErrorMessage)], 200);
    // return trans($finalErrorMessage);

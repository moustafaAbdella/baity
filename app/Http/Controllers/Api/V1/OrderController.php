...
//            'contact_person_number' => $request->contact_person_number ? ($request->user ? $request->contact_person_number :str_replace('+', '', $request->contact_person_number)) : ($request->user?$request->user->phone:''),
        ...
        try {
            ...
            // //to edit deliveryTime corectly
            // dd($order->store->delivery_time);

            $data = [
                'coordinates' => $request->longitude . ', ' . $request->latitude,
                'orderNumber' => $order->id,
                'authorityId' => $order->module->authority_id,
                'category_id' => $order->module->category_id,
                'regionId' => $request->region_id,
                'cityId' => $request->city_id,
                'storetName' => $order->store->name,
                'storeLocation' => $order->store->longitude . ', ' . $order->store->latitude,
                'orderDate' => now()->setTimezone('UTC')->format('Y-m-d\TH:i:s.v\Z'),
                'deliveryTime' => now()->addHours(6)->setTimezone('UTC')->format('Y-m-d\TH:i:s.v\Z'),
                'recipientMobileNumber' => $request->recipientMobileNumber,
            ];

            $newRequest = new Request($data);
            // استدعاء دالة createorder
        $order_auth = $this->create_order_to_authority($newRequest);


        $responseContent = $order_auth->getContent();

$data = json_decode($responseContent);
$order->referenceCode = $data->data->referenceCode;
$order->update();

$newRequestq = new Request([
    'referenceCode' => $order->referenceCode,
]);
    $order_au = $this->get_order_from_authority($newRequestq);
dd($order_au);

        $responsedata = json_decode($order_auth->getContent(), true);
...
if (isset($responsedata['status']) && $responsedata['status'] === true) {
            return response()->json([
                ...
                // 'refrenceCode' => (int) $order->user_id,
            ], 200);
        }
        } catch (\Exception $e) {
            DB::rollBack();
...
//        })
    ...
    public function create_order_to_authority(Request $request)    {
        ...
        $response = Http::post('https://demo-apitawseel.naql.sa/api/Order/create', [
            'credential' => [
                'companyName' => 'master-taer',
                'password' => 'FRgQKCtIDClc',
            ],
